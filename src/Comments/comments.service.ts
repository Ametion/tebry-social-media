...
export class CommentsService{
    ...
    public async EditComment(editCommentDTO: EditCommentDTO): Promise<boolean | ResponseModel> {
        try{
            if(!await this.usersService.IsUserExist(editCommentDTO.login)){
                return new ResponseModel(204, "No user with this login");
            }

            if(!await this.usersService.CheckToken(editCommentDTO.login, editCommentDTO.token)){
                return new ResponseModel(400, "Wrong access token");
            }

            const comment = await CommentsRepo.findOneOrFail({
                where: {
                    id: editCommentDTO.commentId
                }
            })

            if(!editCommentDTO.newCommentContent){
                return false;
            }

            comment.content = editCommentDTO.newCommentContent;
